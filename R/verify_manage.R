...
VerifyManage <- R6::R6Class( "VerifyManage",
    ...
    get_machine_id = function() {
      ...
      tryCatch({
        if (!is.null(self$dll_name)) {
          ...
          # cat("动态库调用成功，返回机器码:", machine_id)  # 生产环境注释
          return(machine_id)
        } else {
          # 使用模拟函数
    ...
    verify_license = function(license_key = NULL) {
      tryCatch({
        if (is.null(license_key)) {
          # 尝试从文件读取授权码
          stored_license <- private$read_license()
        ...
        tryCatch({
          ...
          if (status_code == 200) {
            ...
            # print(content)  # 生产环境注释
            if (!is.null(content$success) && content$success) {
              if (!is.null(license_key)) {
                private$save_license(license_key)
              }
  ...
  private = list(
    verify_url = "https://www.medsta.cn/apis/encryption.halo.run/v1alpha1/rpackagelicenses/verify",
    license_file = NULL,
